...
static int makeindex (unsigned char * file_buf,int size){
 
  unsigned int tmp_p;//temp position
	unsigned char i,j;
	CHAIN * index_p;
//for malloc efficiency!!!!
	// CHAIN * chain_start;
	
	// chain_start = (CHAIN *)calloc(65536,sizeof(CHAIN));
	// for(ii=0;ii!=256;ii++)
		// for(jj=0;jj!=256;jj++){
			// hash_index[ii][jj] = chain_start + 256*ii+jj;
			// hash_index[ii][jj]->next=NULL;
		// }
//for malloc efficiency!!!!
	...
	// for(ii=0;ii!=256;ii++)
		// for(jj=0;jj!=256;jj++){
			// // printf("i=%d,j=%d",i,j);
			// hash_index[ii][jj]=(CHAIN *)malloc(sizeof(CHAIN));
			// hash_index[ii][jj]->next=NULL;
		// }
	
	//printf("cleared_index\n");
	for (tmp_p=0;tmp_p < size-1;tmp_p++){
		i=(unsigned char)(*(file_buf+tmp_p));
		j=(unsigned char)(*(file_buf+tmp_p+1));
	//	printf("#i=%u,j=%u#",i,j);
		index_p = &hash_index[i][j];
		//insert new position
		while (index_p->next !=	NULL){
			index_p=index_p->next;
		}
		index_p->next=calloc(1,sizeof(CHAIN));
...
static int freeindex(){
	...
	for(ii=0;ii!=256;ii++)
		for(jj=0;jj!=256;jj++){
			if ((index = (hash_index[ii][jj]).next) == NULL)
				continue;
	...
	memset(hash_index,0,65536*sizeof(CHAIN));//256*256*8
	return(0);
}
// static int freeindex(CHAIN **index){
	// int ii,jj;
	// CHAIN * index_free;
	// CHAIN * tmp_index;
	...
	// for(ii=0;ii!=65536;ii++){
		// tmp_index = *(index+ii);
		// while (tmp_index->next != NULL){
			// index_free = tmp_index;
...
static CODE * genrawcode(void){
	...
	while((chain_counter < MAX_CODE_CHAIN_LEN)&&(window_back < file_read_buf1+read_num-1)){
		...
		if ((hash_index[i][j]).next == NULL) { //no match"only match itself"
			code_type=1;
			repeat_times=1;
		}
...
static void help(){
	printf(
		...
		"     zax foo                    #zax will judge the usage.if 'foo' is a zaxed file zax will uncompress it"
		"                                 else zax will compress it with name foo.zax"
...
int main(int argc, char **argv){
	...
	// drag'n'drop & single-argument mode   JUDGING THE USAGE
	...
	instream_buf = (char *)malloc(FILE_BUF_SIZE1*640);  //FILE_BUF_SIZE1=128 kb malloc=80mb
	...
	if (usage=='c')
	{
		...
		if ( 3 != fwrite(fileheader,1,3,outputfd))
			errquit("fwrite error 1 !\n");
		if ( 1 != fwrite(&file_name_len,1,1,outputfd))
			errquit("fwrite error 2 !\n");
